...
export default function CrudManager({ title, table, rows: initialRows, columns, displayColumns }: {
  ...
  const td = { padding: "0.75rem 1rem", borderBottom: "1px solid rgba(255,255,255,0.05)", color: "rgba(240,237,232,0.8)", fontSize: "0.82rem" };
  const inp = { padding: "0.4rem 0.75rem", borderRadius: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", color: "#f0ede8", fontSize: "0.82rem", outline: "none", fontFamily: "sans-serif", width: "100%" };
  ...
  const allDisplayCols = [...columns, ...displayColumns.filter(c => !columns.includes(c))];
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap", alignItems: "flex-end" }}>
        {columns.map(c => (
          <div key={c} style={{ flex: 1, minWidth: 160 }}>
            <label style={{ fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.08em", color: "rgba(240,237,232,0.4)", display: "block", marginBottom: "0.25rem" }}>{c.replace(/_/g, " ")}</label>
            <input
              value={newData[c] ?? ""}
        ...
        <button onClick={handleCreate} style={{ ...btn("#ff4d00", "#fff"), padding: "0.5rem 1rem", alignSelf: "flex-end" }}>+ Add</button>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {allDisplayCols.map(c => <th key={c} style={th}>{c.replace(/_/g, " ")}</th>)}
